...
server<-function(input, output, session){
  ...
  rename_transformation_colum<-function(column, transformation_list){
    t_name<-stringr::str_flatten(transformation_list)
    colnames(column)<-paste(t_name,"(",colnames(column),')',sep="")
    return(column)
  }
  observeEvent(input$accept_response_transformation,{
    ...
    if (is.null(rv())){
      ...
      rv(rename_transformation_colum(newcol,input$t_type_response))
      
    }else{
      old_rv<-rv()
      old_rv<-old_rv%>%select(-contains(as.character(input$response_var)))
      #colnames(newcol)<-paste(input$t_type_response,'(',colnames(newcol),')',sep = '')
      #message(colnames(newcol))
      
      rv(cbind(old_rv,rename_transformation_colum(newcol,input$t_type_response)))
      #message('rv is not null')
    }
  })
  observeEvent(input$accept_x1_transformation,{
    newcol<-df_transform()[1]
    if (is.null(rv())){
